        ...
        <form action="{{ route('update_barang', $barang->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nama_barang" id="nama_barang"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required value="{{ old('nama_barang', $barang->nama_barang) }}">
            </div>
            <div>
                <label for="stok" class="block text-sm font-medium text-text-white">Stok Barang</label>
                <input type="number" name="stok" id="stok"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required value="{{ old('stok', $barang->stok) }}">
            </div>
            <div>
                <label for="keterangan" class="block text-sm font-medium text-text-white">Keterangan</label>
                <input type="text" name="keterangan" id="keterangan"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('keterangan', $barang->keterangan) }}">
            </div>
